    ...
    for line in f:
        ...
        if len(line) == 0:
            ...
            for sentence in data:
                ...
                #because of broken extracting lines are split,
                # ignore 'sentence' that start with a loercase letter.
                if not sentence[0].islower():

                    #make sure the 'sentence actally ends with a punctuation mark.
                    if sentence[len(sentence)-1] in ["." , "?" , "!"]:

                        #a lot of sentences have a number as a gab (or reference to a page), ignore them.
                        if not re.search(r'\d', sentence):

                            #some are just a punctuation mark.
                            if len(sentence) > 1:
